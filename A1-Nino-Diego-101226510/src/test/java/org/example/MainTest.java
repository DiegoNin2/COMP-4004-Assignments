...
class MainTest {
    ...
    @Test
    @DisplayName("Check if player is prompted to leave hot seat after turn ends")
    void RESP_05_test_04() {
        Main game = new Main();
        game.initializeDecks();
        game.initializePlayers();

        //game should tell player to leave after printing everything out
        StringWriter output = new StringWriter();

        game.takeTurn(new PrintWriter(output));

        assertTrue(output.toString().contains("Turn end. Please leave the hot seat for next player."));
    }

    @Test
    @DisplayName("Check if game cycles through all 4 players & displays their id in order")
    void RESP_05_test_05() {
        Main game = new Main();
        game.initializeDecks();
        game.initializePlayers();

        //game should display each player id in the order of P1 > P2 > P3 > P4 > P1
        StringWriter output = new StringWriter();

        game.takeTurn(new PrintWriter(output));

        assertAll(
                "player id output check",
                () -> assertTrue(output.toString().contains("Current Player: P1")),
    ...
    @Test
    @DisplayName("Check if game cycles through all 4 players & displays their hand in order")
    void RESP_05_test_06() {
        Main game = new Main();
        game.initializeDecks();
        game.initializePlayers();

        //game should display each player's hand in the order of P1 > P2 > P3 > P4 > P1
        StringWriter output = new StringWriter();

        game.pickCard(0,"Weapon","Dagger", "5", 1);
        game.pickCard(1,"Foe","F5", "5", 1);
        game.pickCard(0,"Weapon","Horse", "10", 2);
        game.pickCard(1,"Weapon","Sword", "10", 2);
        game.pickCard(0,"Weapon","Excalibur", "30", 3);
        game.pickCard(1,"Weapon","Dagger", "5", 3);
        game.pickCard(2,"Weapon","Dagger", "5", 3);
        game.pickCard(0,"Foe","F50", "50", 4);
        game.pickCard(1,"Foe","F50", "50", 4);
        game.pickCard(2,"Foe","F70", "70", 4);

        game.takeTurn(new PrintWriter(output));

        assertAll(
                "hand display check",
                () -> assertTrue(output.toString().contains("F5, value = 5 \nDagger, value = 5")),
                () -> assertTrue(output.toString().contains("Sword, value = 10 \nHorse, value = 10")),
                () -> assertTrue(output.toString().contains("Dagger, value = 5 \nDagger, value = 5 \nExcalibur, value = 30")),
                () -> assertTrue(output.toString().contains("F50, value = 50 \nF50, value = 50 \nF70, value = 70"))
        );
    }
